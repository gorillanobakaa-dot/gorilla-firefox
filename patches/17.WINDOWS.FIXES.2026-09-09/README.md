...
Each item has a matching check in the Windows build kit, so it is caught
if it comes back.
...
- `browser/components/BrowserGlue.sys.mjs`  
  calls the registration from _onFirstWindowLoaded. The lazy getter must use resource:///modules/, NOT moz-src:///browser/modules/ - the wrong scheme throws and the extension silently vanishes
...
- `browser/extensions/moz.build`  
  adds the bundled-extension directory to DIRS
...
- `browser/modules/moz.build`  
  registers that module (EXTRA_JS_MODULES is sorted case-insensitively)
...
- `browser/modules/GorillaBuiltinExtensions.sys.mjs`  
  registers bundled extensions via maybeInstallBuiltinAddon (the VISIBLE app-builtin location) and pins the toolbar button once; generated by windows/add_builtin_extension.py
